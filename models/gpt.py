...
def gpt_completion_fn(model, input_str, steps, settings, num_samples, temp):
    ...
    if (model not in ['gpt-3.5-turbo','gpt-4','gpt-4-1106-preview','gpt-3.5']): # logit bias not supported for chat models
        logit_bias = {id: 30 for id in get_allowed_ids(allowed_tokens, model)}
    if model in ['gpt-3.5-turbo','gpt-4','gpt-4-1106-preview','gpt-3.5']:
